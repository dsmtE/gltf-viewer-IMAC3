...
namespace gltfUtils {
  ...
  std::vector<GLuint> createVertexArrayObjects(const tinygltf::Model& model, const std::vector<GLuint>& bufferObjects, 
    ...
    for (size_t i = 0; i < model.meshes.size(); ++i) {
      ...
      // gen Vertex arrays for our new mesh primitives
      GLCALL(glGenVertexArrays(vaoRange.count, &vertexArrayObjects[vaoRange.begin]));
      ...
      for (size_t primitiveIdx = 0; primitiveIdx < mesh.primitives.size(); ++primitiveIdx) { // for each primitives
        ...
        for(const auto &attributInfos : attributesNamesAndIndex) { // for each attributs
          ...
          if (iterator != end(primitive.attributes)) {
            ...
            // bind to our previous generated buffer Object
            GLCALL(glBindBuffer(GL_ARRAY_BUFFER, bufferObjects[bufferIdx]));
          
            const size_t byteOffset = accessor.byteOffset + bufferView.byteOffset;
            // https://www.khronos.org/registry/OpenGL-Refpages/gl4/html/glVertexAttribPointer.xhtml
            GLCALL(glVertexAttribPointer(attributIdx, accessor.type, accessor.componentType, GL_FALSE, static_cast<GLsizei>(bufferView.byteStride), (const GLvoid *)byteOffset));
