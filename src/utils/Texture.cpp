...
GLuint Texture::CreateTextureID(const GLint interpolationMode, const std::array<GLint, 3>& wrapsMode) {
    GLuint textureId;
    ...
    if (std::any_of(mipmapMode.begin(), mipmapMode.end(), [&](GLint i){return i == interpolationMode;})) {
        glBindTexture(GL_TEXTURE_2D, 0);
    }
	return textureId;
}

void Texture::genTexture(const GLint interpolationMode, const std::array<GLint, 3>&  wrapsMode) {
#ifndef DEBUG
	if (textureId_ != -1){
...
#endif
    textureId_ = CreateTextureID(interpolationMode, wrapsMode);
}
